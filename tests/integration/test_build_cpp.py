...
def test_build_uses_and_reports_cpp_payload_language(
    ...
    (project / "src").mkdir()
    (project / "src/payload.inc").write_text(
        "template<int N> struct Add { static int apply(int x) { return x + N; } };\n"
        'extern "C" int rommod_payload(int x) { return Add<5>::apply(x); }\n',
        encoding="utf-8",
    )
    ...
    assert get_main_binary(rebuilt, "arm9")[4:8] != bytes.fromhex("05 06 07 08")

    report = json.loads(result.report_path.read_text(encoding="utf-8"))
    assert report["changes"][0]["language"] == "cpp"
