...
def test_build_uses_and_reports_c_preprocessor_defines(
    ...
    (project / "src/payload.c").write_text(
        ...
        "#error missing ROMMOD_MULTIPLIER\n"
        "#endif\n"
        "int rommod_payload(int x) { return x * ROMMOD_MULTIPLIER; }\n",
        encoding="utf-8",
    )
    ...
    assert report["changes"][0]["defines"] == ["ROMMOD_MULTIPLIER=3"]
